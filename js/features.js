...
// ===== 番茄钟入口 =====
document.addEventListener('DOMContentLoaded', function() {
    var pomoEntry = document.getElementById('pomodoro-function');
    if (pomoEntry) {
        pomoEntry.addEventListener('click', function() {
            var advModal = document.getElementById('advanced-modal');
            if (advModal && typeof hideModal === 'function') {
                hideModal(advModal);
            }
            if (typeof window.pomodoro !== 'undefined') {
                window.pomodoro.open();
            } else {
                if (typeof showNotification === 'function') {
                    showNotification('番茄钟模块加载中，请稍后...', 'info');
                }
            }
        });
    }
